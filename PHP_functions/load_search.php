      ...
      $query = "SELECT id, title, description_c, images_path, category_id, author, id_us FROM $tabella WHERE status_bool = 2 AND (title LIKE '%$query%' OR description_c LIKE '%$query%' OR subtitle LIKE '%$query%' OR author LIKE '%$query%' OR link LIKE '%$query%') ORDER BY boost_start DESC, views DESC LIMIT " . $_GET['start'] . ", " . $_GET['tot'];
