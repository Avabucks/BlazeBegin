    ...
    $tabella = 'tbl_user';
    $username_us = str_replace("-", " ", $_GET['profile']);
    ...
    $query = "SELECT id_us, username_us FROM $tabella WHERE username_us = '$username_us'";
...
<head>
  ...
  <title><?php echo $username_us ?> - BlazeBegin</title>
...
<body>
    ...
    <div class="title">
      <div>
        <div class="breadcrumb">
          ...
          <a href="<?php echo $folder ?>/explore">Explore</a>
          <i class='bx bx-chevron-right'></i>
          <span><?php echo $username_us ?></span>
        </div>
        <span>
          <i class='bx bxs-user' ></i>
          <h1><?php echo $username_us ?></h1>
        </span>
        <div class="underline"></div>
      </div>
      <h2>This is the page that contains all the startups founded by "<?php echo $username_us ?>". Browse and choose the startup that's right for you!</h2>
    </div>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $.ajax({
        type: "GET",
        url: "../PHP_functions/load_profile", 
        data: {"start":start, "tot":tot, "id_us":"<?php echo $id_us ?>"},
        success: function(response) {
          document.querySelector("#profile-tab div").innerHTML += response;

          if (document.querySelector("#profile-tab div").children.length == 0) {
          ...
          + '</div>';
            document.querySelector("#profile-tab").innerHTML = response;
          }

          if (document.querySelector("#profile-tab div").children.length < 4)
            document.querySelector("#profile-tab div").innerHTML += '<div class="hide-on-mobile"></div><div class="hide-on-mobile"></div><div class="hide-on-tablet hide-on-mobile"></div><div class="hide-on-tablet hide-on-mobile"></div>';
          ...
          $.ajax({
            type: "GET",
            url: "../PHP_functions/load_profile", 
            data: {"start":start + tot, "tot":tot, "id_us":"<?php echo $id_us ?>"},
            success: function(response) {
              if (response.trim() == "") {
